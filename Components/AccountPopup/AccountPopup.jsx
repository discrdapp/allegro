...
const AccountPopup = props => {
	// Sign-out handler updates localStorage and props
	const signOutHandler = () => {
		localStorage.removeItem('currentUser')
		props.setIsLoggedIn(false)
		props.setCurrentUserEmail(null)
	}

	return ReactDOM.createPortal(
		<>
			{props.isLoggedIn === true ? (
				<div className={styles.container} style={{ top: `${props.offsetHeight}px` }}>
				...
				<div className={styles.container} style={{ top: `${props.offsetHeight}px` }}>
					...
					<Link to='/logowanie' style={{ width: '100%' }}>
						<Button value={'ZALOGUJ SIĘ'} style={{ margin: '15px 0', width: '100%', padding: '.7em 0' }} />
					</Link>
					<p>
						Nie masz konta?{' '}
