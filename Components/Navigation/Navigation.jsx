				...
				{props.blank !== true && (
					<>
						...
						<div className={styles.iconsWrapper}>
							...
							<a href='#home' onMouseEnter={enterHandler} onMouseLeave={leaveHandler} ref={cartIconRef}>
								<img src={cartIcon} className={styles.cartIcon} alt='Ikonka koszyka' />
								{cartContext.currentTotalItems > 0 ? (
									<span className={styles.totalItemsInCart}>{cartContext.currentTotalItems}</span>
								) : (
									''
								)}
							</a>
							{isVisibleModal && (
								<CartModal offsetHeight={cartOffset} isMouseOverOnModal={setIsMouseoverOnModal} timeoutId={timeoutId} />
							)}
							<img src={userIcon} alt='Ikonka użytkownika' className={styles.loginIcon} onClick={accountClickHandler} />
							{isMyAllegroClicked && (
								<AccountPopup
							...
							<div className={styles.myAllegro} onClick={accountClickHandler}>
								...
								<p className={styles.currentUserName}>{currentUserEmail ? currentUserEmail : 'Moje Allegro'}</p>
								<img
									src={arrow}
									alt='Strzałka'
									className={isMyAllegroClicked ? styles.rotateArrow : styles.actionArrow}
								/>
							</div>
						</div>
					</>
